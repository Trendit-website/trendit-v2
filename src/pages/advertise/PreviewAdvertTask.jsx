...
export default function PreviewAdvertTask() {
  ...
  const getPreview = (key) => {
    ...
    API.get(`user/tasks/${key}`)
      .then((response) => (setPreview(response.data?.task), isLoading(false), toast.success(response.data.message), console.log(response)))
                ...
                {taskPreview?.status === 'pending' && (
                  <div className='grow shrink basis-0 justify-start items-center gap-2.5 flex'>
                    <div className="grow shrink basis-0 text-blue-600 text-xs font-normal font-['Manrope']">
                      Your task has not been approved and is not on our page for
            ...
            <div className='w-full flex flex-col items-center pl-2 gap-y-6 pt-10 lg:pl-0'>
                <div className='w-full flex items-center justify-between'>
                    <div className='w-10/12 flex flex-col gap-y-2'>
                      <p className='text-xs font-semibold'>Numbers of Followers</p>
                      <Input type='text' value={taskPreview?.engagements_count}className='w-11/12' disabled/>
                    </div>
                    <div className='w-10/12 flex flex-col gap-y-2'>
                      <p className='text-xs font-semibold'>Location</p>
                      <Input type='text' value={taskPreview?.location}className='w-11/12' disabled/>
                    </div>
                </div>
                <div className='w-full flex items-center justify-between'>
                    <div className='w-10/12 flex flex-col gap-y-2'>
                      <p className='text-xs font-semibold'>Your Link</p>
                      <Input type='text' value={taskPreview?.account_link}className='w-11/12' disabled/>
                    </div>
                    <div className='w-10/12 flex flex-col gap-y-2'>
                      <p className='text-xs font-semibold'>Gender</p>
                      <Input type='text' value={taskPreview?.gender}className='w-11/12' disabled/>
                    </div>
                </div>
                <div className='w-full flex items-center justify-between'>
                    <div className='w-10/12 flex flex-col gap-y-2'>
                      <p className='text-xs font-semibold'>Religion</p>
                      <Input type='text' value={taskPreview?.religion}className='w-11/12' disabled/>
                    </div>
                    <div className='w-10/12 flex flex-col gap-y-2'>
                      <p className='text-xs font-semibold'>Amount paid</p>
                      <Input type='text' value={taskPreview?.fee_paid}className='w-11/12' disabled/>
                    </div>
                </div>
                <div className='w-full flex items-center justify-between'>
                    <div className='w-10/12 flex flex-col gap-y-2'>
                      <p className='text-xs font-semibold'>Allocated</p>
                      <Input type='text' value={taskPreview?.total_allocated}className='w-11/12' disabled/>
                    </div>
                    <div className='w-10/12 flex flex-col gap-y-2'>
