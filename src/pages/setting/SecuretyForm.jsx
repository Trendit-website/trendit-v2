...
function SecuretyFormContent() {
  ...
  const { data: activeGoogleAuth, isPending } = useActivateGoogleAuth()
  // const { data: de_activeGoogleAuth } = useDeactivateGoogleAuth()
  const { isOpen, onOpen, onClose } = useDisclosure()
  const [googleAuthActive, setGoogleAuthActive] = useState(false) // Managed based on the response or initial data load
  ...
  console.log(activeGoogleAuth, 'google')
  ...
  const handleToggleGoogleAuth = async () => {
    try {
      ...
      if (res?.data) {
        ...
        onOpen() // Open the modal only on successful activation
    ...
    queryClient.invalidateQueries(['security_preference'])
  }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const res = await handleSecurityUpdate({
        data,
        // dada: { two_fa_method: enabled ? 'email' : null },
      })
      if (res?.data?.status) {
        toast.success(res.data.message)
        queryClient.invalidateQueries({ queryKey: ['sec_prefence'] })
  ...
  return (
    <>
                      ...
                      <Input
                        ...
                        endContent={
                          <Switch
                            ...
                            // onChange={(e) =>
                            //   handleSubmit(onSubmit)(e.target.checked)
                            // }
                            color='secondary'
                          />
                        }
                  ...
                  <Input
                    ...
                    endContent={
                      ...
                      >
                        ...
                        {userDetails?.enabled ? 'Deactivate' : 'Activate'}
                      </Button>
                    }
      ...
      {isOpen && (
        <QrCodeModal
          activeGoogleAuth={activeGoogleAuth}
          isOpen={isOpen}
          onClose={onClose}
        />
      )}
